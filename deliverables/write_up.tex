...
\subsection{4. The AI Prototype: Advanced Multi-Agent Architecture}\label{the-ai-prototype-advanced-multi-agent-architecture}

To deliver a production-ready solution, I developed \texttt{app.py}, a \textbf{Streamlit Web Application} showcasing an advanced Multi-Agent architecture powered by \texttt{gemini-3.5-flash}.
...
\subsubsection{Operational Safety: Risks \& Mitigations}\label{operational-safety-risks-mitigations}
Deploying AI in financial workflows carries inherent risks (e.g., hallucinated refund amounts or improper authorizations). Our architecture mitigates this using a \textbf{Human-In-The-Loop (HITL) Design}:
\begin{itemize}
\tightlist
\item \textbf{Financial Compliance}: The AI is strictly limited to \textit{drafting} explanatory emails and structuring incoming data. It cannot authorize payouts. A human Support representative must click "Approve \& Send" on the AI's drafted email before any external communication occurs.
\item \textbf{Secure Integration (MCP)}: In production, the LLM will bridge with the internal SSOT database via the \textbf{Model Context Protocol (MCP)}. This guarantees that raw database credentials are never exposed to the LLM's context window.
\end{itemize}
