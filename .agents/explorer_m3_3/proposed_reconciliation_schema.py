...
class DraftReconciliationMessageRequest(BaseModel):
    ...
    agent: str = Field(..., description="Partner agency name")
    route: Optional[str] = Field(None, description="Flight sector/route (e.g. DEL-DXB)")
    support_amount: float = Field(..., description="Amount promised to customer (INR)")
    finance_amount: float = Field(..., description="Amount paid by finance (INR)")
    deduction: float = Field(..., description="Deduction amount (INR)")
    reason: Optional[str] = Field(None, description="Carrier penalty or remarks")


class DraftReconciliationMessageResponse(BaseModel):
